...
def compute_daily_returns(price_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        price_df is None
        or price_df.empty
        or not required_cols.issubset(price_df.columns)
    ):
...
def generate_result_summary(
    ...
    for r in results[:5]:  # Limit to first 5 results
        ...
        end_price = f"{r['end_price']:.2f}" if r.get("end_price") is not None else "n/a"
        ...
        cr = (
            f"{r['current_ratio']:.2f}" if r.get("current_ratio") is not None else "n/a"
        )
        mc = f"{r['market_cap']:.0f}" if r.get("market_cap") is not None else "n/a"
        vol = f"{r['volatility']:.4f}" if r.get("volatility") is not None else "n/a"
...
def integrate(
    ...
    required_cols = {"symbol", "trade_date", "close_price"}
    if (
        price_for_risk is None
        or price_for_risk.empty
        or not required_cols.issubset(price_for_risk.columns)
    ):
        try:
            ...
            if hasattr(plan, "warnings"):
                plan.warnings.append(
                    f"Could not fetch daily prices for risk metrics: {e}"
                )
